...
select region,sum(price*quantity) as total_rvenue from ecommerce_orders group by region;
select order_id,customer_name,price from ecommerce_orders order by price desc limit 5;
--join---
...
case
    ...
    from ecommerce_orders;
    --sub queryes--
    select* from ecommerce_orders where price >
    ...
    sum(quantity*price) as total_revenue from ecommerce_orders
    group by month order by month;
    
    select product,sum(quantity) as total_quantity from ecommerce_orders group by product order by total_quantity desc limit 5;
    
    select category,
    sum(quantity*price) as revenue from ecommerce_orders group by category;
